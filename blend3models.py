...
def main(low_res_pkl: Path, # Pickle file from which to take low res layers
         medium_res_pkl: Path, # Pickle file from which to take high res layers
         high_res_pkl: Path, # Pickle file from which to take high res layers
         resolution1: int, # Resolution1 level at which to switch between models
         resolution2: int, # Resolution2 level at which to switch between models
    ...
    with tf.Session() as sess, tf.device('/gpu:0'):
            ...
            medium_res_G, medium_res_D, medium_res_Gs = misc.load_pkl(medium_res_pkl)
            high_res_G, high_res_D, high_res_Gs = misc.load_pkl(high_res_pkl)

            out = blend_models(low_res_Gs, medium_res_Gs, resolution1, level, blend_width=blend_width, verbose=verbose)
            out = blend_models(out, high_res_Gs, resolution2, level, blend_width=blend_width, verbose=verbose)
